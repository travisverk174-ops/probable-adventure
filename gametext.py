...
import time
import os
def print_slowly(text, color=Fore.RED, style=""):
    for chr in text:
       print(f"{style}{color}{chr}{Style.RESET_ALL}", end="", flush=True)
       time.sleep(0.0005)


def intro_to_story():
    a = ("In the cheerful lands of the Mushroom Kingdom, Mario was enjoying a quiet day.\n\
...
def sewer_hat():
    segment = '+10 Armor'
    a = (f"Mario turned left, drawn by the soft red glow in the distance.\n\
...
The hat, now infused with something ancient and resilient, hardened his resolve and his body. ")
    print_slowly(a)
    print_slowly(segment, Fore.WHITE, Style.BRIGHT)
    print("\n")

def sewer_wpn_pipe():
    segment = '+10 Strength'
    a = (f"Mario turned right, drawn by the object in the water.\n\
...
It was a message: Mario wasn’t here to run or dodge. He was here to fight. ")
    print_slowly(a)
    print_slowly(segment, Fore.WHITE, Style.BRIGHT)
    print("\n")

def text_after_sewer():
    segment = 'A corrupted Goomba.'
    a = (f"Mario pressed onward, the narrow tunnel forcing him to crouch beneath dripping pipes and weave around loose stone.\n\
...
Red pits glowed where eyes should be, and jagged teeth twitched in its mouth.")
    print_slowly(a)
    print_slowly(segment, Fore.RED, Style.BRIGHT)
    print_slowly(b)
    print("\n")


def goomba_defeated():
    a = (f"The corrupted Goomba collapsed into the sludge, its twisted form dissolving into black mist.\n\
The chamber fell silent once more, but the air remained heavy, as if the fight had stirred something deeper.\n\
       ...
       \nTo the left, a narrow tunnel stretched into the distance.\n\
...
def choice_after_goomba_garden():
    a = (f"Mario stepped out of the sewer and into sunlight.\n\
...
The path curved gently, leading him to another fork.\n\
        \nTo the left, a glasshouse shimmered in the sun.\n\
...
def garden_left1():
    segment = '+5 Damage'
    a = (f"Mario stepped into the glasshouse.\n\
...
Something about it felt… important.\n\
                  ...
                  \nA rush of heat surged through him. His grip steadied. His senses sharpened. ")
    print_slowly(a)
    print_slowly(segment, Fore.WHITE, Style.BRIGHT)
    print("\n") 

def garden_left2():
       a = (f"He stepped back through the cracked doorway, leaving behind the jungle of alien flora.\n\
...
def garden_right():
    segment1 = 'It exploded open.\nA Piranha Plant'
    segment2 = ' shot out, its jaws snapping inches from his face.'
    a= (f"Mario stepped off the garden path and approached the compost bin.\n\
...
Something wasn’t right.\n\
    ...
    b = ("This wasn’t the kind he’d faced before.\n\
...
Smarter. Meaner. And it wasn't going to wait.")
    print_slowly(a)
    print_slowly(segment1, Fore.RED, Style.BRIGHT)
    print_slowly(segment2)
    print_slowly(b)
    print("\n")

def piranha_defeated():
    segment = 'castle.'
    a = (f"The Piranha Plant let out a final hiss before collapsing into the compost, its vines twitching once before going still.\n\
...
def choice_after_goomba_prison():
    segment1 = 'pleading.'
    segment2 = 'The prison chambers.'
    a = (f"Mario turned away from the faint light and stepped deeper into the tunnel.\n\
...
A broken lantern flickered in the corner, casting shadows that danced like figures.\n\
              \nThen he heard it.\n\
              \nA voice. Clearer now. Not whispering but ")
...
Chains hung from the ceiling, and a single torch burned weakly near the far wall.\n\
    ...
    print_slowly(segment1, Fore.RED, Style.BRIGHT)
    print_slowly(b)
    print_slowly(segment2, Fore.RED, Style.BRIGHT)
    print_slowly(c)
    print("\n")


def continuation_prison():
    a = (f"Mario stepped deeper into the prison chambers.\n\
...
At the far end of the chamber, two passageways opened — one to the left, one to the right.\n\
            \nTo the left, the corridor was lined with cracked tiles and faded signs.\n\
...
Somewhere down that hall, a door creaked open and shut, as if caught in a slow breeze.\n\
    \nTo the right, the stone turned darker.\n\
...
def prison_pharmacy1():
    segment = 'HP restored'
    a = (f"Mario turned toward the corridor marked by the faded red cross.\n\
...
The tension eased. His breath came easier.\n")
    ...
    c = ("\nMario scanned the room once more. It felt untouched, like someone had prepared it for him long ago.")
    print_slowly(a)
    print_slowly(segment, Fore.WHITE, Style.BRIGHT)
    print_slowly(c)
    print("\n")
...
def prison_pharmacy2():
    a = (f"Mario stepped out of the treatment room, the mushroom’s warmth still pulsing through him.\n\
...
def prison_mortuary():
    segment1 = 'burned remains '
    segment2 = '“Purification Chamber” '
    segment3 = 'Something moved.'
    segment4 = 'A Boo.'
    a = (f"Mario stepped into the right corridor.\n\
...
Its eyes locked onto Mario, and its mouth twisted into a mischievous sneer.\n")
    ...
    print_slowly(segment1, Fore.RED, Style.BRIGHT)
    print_slowly(b)
    print_slowly(segment2, Fore.WHITE, Style.BRIGHT)
    print_slowly(c)
    print_slowly(segment3, Fore.RED, Style.BRIGHT)
    print_slowly(d)
    print_slowly(segment4, Fore.RED, Style.BRIGHT)
    print("\n")


def boo_defeated():
    a = ("The Boo let out a final shriek before vanishing into wisps of pale smoke.\n\
...
def before_castle_door():
    ...
    segment1_5 = '\nBowser: '
    segment2 = '\nMario: '
    a = (f"Mario stood before the massive door, its silver bands pulsing with arcane energy.\n\
...
His face was barely recognizable — stretched, cracked, and crowned with horns that scraped the mist above.\n\
    ...
    c = ("'You came through the veins of my body, Mario. This castle... this world... it is me now.'\n\
    \nHis voice was thunder, distorted and layered, echoing from every wall.\n")

    d = ("'You’re not Bowser anymore.'\n")
    e = ("'I am what you made me. Every defeat. Every humiliation. I fed on it.\n\
I grew. And now, I am beyond kingship. I am wrath incarnate.'\n\
    ...
    f = ("'Then I’ll tear this rage out by the roots.'")
    print_slowly(a)
    print_slowly(segment1, Fore.RED, Style.BRIGHT)
    print_slowly(b)
    print_slowly(segment1_5, Fore.RED, Style.BRIGHT)
    print_slowly(c)
    print_slowly(segment2, Fore.GREEN, Style.BRIGHT)
    print_slowly(d)
    print_slowly(segment1_5, Fore.RED, Style.BRIGHT)
    print_slowly(e)
    print_slowly(segment2, Fore.GREEN, Style.BRIGHT)
...
The rage was gone. But something deeper remained.\n\
       ...
       \nA name.\n")
    print_slowly(a)
    print_slowly(segment,Fore.MAGENTA,Style.BRIGHT)
    print("\n")